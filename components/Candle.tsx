...
const SmokeParticle: React.FC<{ startPos: [number, number, number]; index: number }> = ({ startPos, index }) => {
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      meshRef.current.position.x = startPos[0] + initialX + Math.sin(time * 2 + index) * 0.02 * (meshRef.current.position.y * 2); 
      meshRef.current.position.z = startPos[2] + initialZ + Math.cos(time * 1.5 + index) * 0.02 * (meshRef.current.position.y * 2);
  ...
  useFrame((state) => {
    ...
    if (flameRef.current && lightRef.current) {
      if (!isLit) {
        ...
        // Active flicker
        const flicker = Math.sin(time * 10) * 0.1 + Math.cos(time * 23) * 0.1 + Math.sin(time * 45) * 0.05;
