...
import { OrbitControls, Stars, PerspectiveCamera, Environment, Text3D, Center } from '@react-three/drei';
...
const MESSAGE_ON_POLAROID = "Make a wish!\n\nMay your year be as\nbright as these stars.\n\nHappy Birthday!";
// --------------------------------
...
const Scene: React.FC<SceneProps> = ({ viewMode, customText, candlesBlownOut = false }) => {
  ...
  const balloons = useMemo(() => {
    const colors = [GOLD_COLOR, CREAM_COLOR, '#ffffff', '#c0c0c0', CHOCOLATE_COLOR];
  ...
  return (
    ...
    >
      <Suspense fallback={null}>
        ...
        <pointLight position={[30, 20, 30]} intensity={2.0} color="#fff8e1" castShadow shadow-mapSize={[2048, 2048]} />
        <pointLight position={[-30, 10, -20]} intensity={1.5} color="#ffd700" />
        <spotLight position={[0, 40, 0]} angle={0.5} penumbra={1} intensity={2.5} color="#ffffff" castShadow shadow-bias={-0.0001} />
        <directionalLight position={[0, 10, 20]} intensity={1.0} color="#fff5e6" />
        ...
        <mesh position={[0, -7.6, 0]} receiveShadow rotation={[-Math.PI / 2, 0, 0]}>
        ...
        <group>
          {balloons.map((b, i) => (
            <Balloon key={i} position={b.position} color={b.color} scale={b.scale} />
          ))}
        </group>
          ...
          <group>
            ...
            <group position={[0, 7.5, 0]}>
              ...
              <group position={[0, 0, 0.5]}>
                ...
                <Rose position={[0.8, 0, -0.2]} scale={1.1} color={GOLD_COLOR} />
                <Rose position={[0, 0.2, 0.8]} scale={1.3} color={GOLD_COLOR} />
                <Rose position={[0, 0.4, -0.5]} scale={1.0} color={GOLD_COLOR} />
              </group>
            </group>
          </group>
