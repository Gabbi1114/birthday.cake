...
const Sprinkles: React.FC<{ radius: number; count: number }> = ({ radius, count }) => {
  const items = useMemo(() => {
    const data = [];
    ...
    for (let i = 0; i < count; i++) {
        ...
        data.push({ position: [x, 0.05, z] as [number, number, number], rotation: [Math.PI / 2, rotY, 0] as [number, number, number], color, type });
    }
    return data;
  }, [radius, count]);
...
const CreamRim: React.FC<{ radius: number; color: string; isBottom?: boolean }> = ({ radius, color, isBottom }) => {
    ...
    return (
        <group>
            {dollops.map((d, i) => (
                 <mesh key={i} position={d.pos} scale={[d.scale, d.scale * 0.8, d.scale]} castShadow>
...
const GoldDrip: React.FC<{ radius: number; height: number }> = ({ radius, height }) => {
    const drips = useMemo(() => {
  ...
  return (
    <group position={position}>
      ...
      <group position={[0, (height / 2) + (withGoldDrip ? 0.06 : 0), 0]}>
         ...
         {withSprinkles && <Sprinkles radius={radius} count={Math.floor(radius * 12)} />}
